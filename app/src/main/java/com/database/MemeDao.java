...
public interface MemeDao {
    ...
    @Query("SELECT meme.* FROM meme LEFT JOIN ranking ON meme.id = ranking.meme_id AND ranking.user_id = :user_id WHERE ranking.user_id is null ORDER BY RANDOM() LIMIT 1")
    MemeDB getUnseenMemeFromUser(Long user_id);
